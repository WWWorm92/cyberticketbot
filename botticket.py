...
@bot.message_handler(content_types=['text', 'document', 'audio', 'photo'])
def message_ticket(msgu):
    if msgu.chat.id == int(chat_id_tickets):
        bot.send_message(chat_id=message_user(msgu.from_user.id),
                         text=msgu.text)  # пересылка ответа поддержки в чат с пользователем
    print('T')
